...
const orderFailureRate = new Rate("foodhub_order_failures");
...
export const options = {
  ...
  thresholds: {
    ...
    foodhub_order_failures: ["rate<0.02"]
  }
};
...
function textSummary(data) {
  ...
  const orderFailures = data.metrics.foodhub_order_failures;

  return [
    "",
    "FoodHub k6 load test summary",
    `http_req_duration p95: ${duration?.values?.["p(95)"] ?? "n/a"} ms`,
    `http_req_failed rate: ${failed?.values?.rate ?? "n/a"}`,
    `foodhub_order_failures rate: ${orderFailures?.values?.rate ?? "n/a"}`,
