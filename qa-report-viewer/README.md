...
The viewer can also start and stop the FoodHub service, show the app/payment Swagger URLs in the main text area, toggle `FOODHUB_AI_LIVE`, and run the QA framework commands from a dropdown. The test runner includes all tests plus the unit, integration, contract, coverage, E2E, load, and report-generation commands used by this project. `All Tests` runs local checks in parallel with fail-fast behavior, and the `Stop` button terminates the active command while releasing spawned resources.
