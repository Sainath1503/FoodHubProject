...
const activeProcesses = new Map<CheckName | "FoodHub", ChildProcess>();
...
let stopping = false;
let startedFoodHubService = false;
...
async function ensureFoodHubService() {
  if (!checks.some((check) => check.needsFoodHubService)) {
    return;
  }

  if (await isHealthy()) {
    console.log("FoodHub service is already running; reusing it for E2E and load checks.");
    return;
  }

  console.log("Starting shared FoodHub service for E2E and load checks.");
  const child = spawnCommand("FoodHub", npm("run", "dev"));
  activeProcesses.set("FoodHub", child);
  startedFoodHubService = true;
  await waitForHealth();
}
...
async function cleanup() {
  for (const [name, child] of activeProcesses.entries()) {
    if (name === "FoodHub" && !startedFoodHubService) {
      continue;
    }
    stopProcessTree(child);
  }
...
async function waitForHealth() {
  for (let attempt = 0; attempt < 30; attempt++) {
    if (await isHealthy()) {
      console.log("Shared FoodHub service is ready.");
      return;
    }
    await new Promise((resolve) => setTimeout(resolve, 1_000));
  }
  throw new Error(`FoodHub service did not become healthy at ${healthUrl}`);
}
...
function selectedChecks() {
  const requested = process.env.FOODHUB_PARALLEL_TEST_CHECKS;
  if (!requested) {
    return allChecks;
  }
  ...
  if (!selected.length) {
    throw new Error(`FOODHUB_PARALLEL_TEST_CHECKS did not match any checks: ${requested}`);
  }
  return selected;
}
