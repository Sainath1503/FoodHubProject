...
const tests: TestCase[] = [
  ...
  test("UNIT-005", "Unit", "tests/unit/recommendationService.test.ts", "AI suggestion recommends a side when the order has a main but no side", 4, "Validates the deterministic AI-style recommendation service."),
  test("UNIT-006", "Unit", "tests/unit/recommendationService.test.ts", "AI suggestion recognizes a balanced order", 4, "Confirms a main, side, and drink combination is treated as balanced."),
  test("UNIT-007", "Unit", "tests/unit/recommendationService.test.ts", "AI suggestion recommends a drink when a main and side are selected", 4, "Covers the main-and-side recommendation branch for Fresh Lemonade."),
  test("UNIT-008", "Unit", "tests/unit/recommendationService.test.ts", "AI suggestion recommends a main when only sides or drinks are selected", 4, "Covers the no-main recommendation branch for Veggie Halloumi Wrap."),
  ...
  test("UNIT-011", "Unit", "tests/unit/paymentService.test.ts", "Fake payment gateway rejects non-gateway tokens", 4, "Directly validates that orders must use tokens returned by FoodHub Payment Gateway."),
